...
import { z } from 'zod'
import {
  normalizeChallengeTelegramUsername,
  upsertChallengeSignup,
} from '@/lib/challenge-signups'
...
const challengeSignupSchema = z.object({
  ...
  telegramUsername: z
    ...
    .min(2)
    .max(120)
    .regex(/^@?[A-Za-z0-9_]+$/, 'invalid_telegram'),
...
export async function POST(request: Request) {
  try {
    ...
    await upsertChallengeSignup({
      name: payload.name,
      telegramUsername: normalizeChallengeTelegramUsername(payload.telegramUsername),
